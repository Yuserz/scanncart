...
def test_local_api_floor_is_lower_than_the_cloud_floor():
    # A local server answered in ~90 ms warm; the cloud took 600-3250 ms.
    # Holding both to one floor would delay every "item left" by 5 s on local.
    from app.settings_store import min_track_expiry_s

    assert min_track_expiry_s("local_api") < min_track_expiry_s("cloud_api")


def test_local_api_accepts_an_expiry_the_cloud_rejects():
    between = 3.0
    assert "too low" not in _warn("local_api", track_expiry_s=between)
    assert "too low" in _warn("cloud_api", track_expiry_s=between)


def test_local_api_still_rejects_an_expiry_below_its_own_floor():
    assert "too low" in _warn("local_api", track_expiry_s=1.0)


def test_unknown_backend_falls_back_to_the_conservative_floor():
    from app.settings_store import MIN_REMOTE_TRACK_EXPIRY_S, min_track_expiry_s

    assert min_track_expiry_s("some_future_backend") == MIN_REMOTE_TRACK_EXPIRY_S
