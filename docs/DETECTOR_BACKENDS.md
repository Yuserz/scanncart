...
| `local_api` | Local Roboflow server | ❌ None | ❌ Free | ✅ Docker / desktop app | ❌ |
...
`settings_store.MIN_REMOTE_TRACK_EXPIRY_S = 5.0` now drives a warning whenever a remote backend
is paired with a lower `track_expiry_s`. **Set `track_expiry_s` to 5.0+ on any remote backend.**
...
| `local_api` selected | Requires `inference server start` running on `local_api_url`. |
